...
def get_deep_args():  
    ...
    parser.add_argument('--gpu', default=default_args['gpu'])
    parser.add_argument('--tune', action='store_true', default=default_args['tune']) 
    parser.add_argument('--objectives', type=int, default=1, choices=[1,2]) 
    ...
    parser.add_argument('--use_float16', action='store_true', default=False,
                    help='Whether to use float16 type for model and data (default: True)')
    args = parser.parse_args()
    ...
    # Here we do the n_estimators and depth
    path = '/pfs/work7/workspace/scratch/ma_fknuette-project_GRANDE/LAMDA-TALENT/TALENT/configs/default/grande_'+args.dataset+'.json'
    with open(path, 'r') as f:
        json_data = json.load(f)

    save_path2 += '-Depth-{}'.format(json_data['grande']['model']['depth'])
    save_path2 += '-Estimators-{}'.format(json_data['grande']['model']['n_estimators'])
    ...
    # load config parameters
    with pkg_resources.files(TALENT).joinpath("configs/default",args.model_type+"_"+args.dataset+'.json').open("r") as f:
        default_para = json.load(f)
