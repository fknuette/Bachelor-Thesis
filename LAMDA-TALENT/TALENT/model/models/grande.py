...
class GRANDE(nn.Module):
    ...
    def forward(self, inputs):
        ...
        if self.task_type == 'regression':
            layer_output = torch.einsum('elc,bel->bec', self.leaf_classes_array.float(), p.float()).half()
        ...
        if self.data_subset_fraction < 1.0 and training:
            ...
            if self.task_type in ['regression', 'binclass']:
        ...
        if self.task_type == 'regression' and result.ndim == 1:
    ...
    def build_model(self):
        ...
        self.split_values = nn.Parameter(torch.randn([self.n_estimators, self.internal_node_num_, self.selected_variables]).to(dtype=torch.float16))
        self.split_index_array = nn.Parameter(torch.randn([self.n_estimators, self.internal_node_num_, self.selected_variables]).to(dtype=torch.float16))
