...
class GRANDEMethod(Method):
    ...
    def construct_model(self, model_config = None):
        ...
        self.model = GRANDE(
                ...
                num_objectives=self.args.objectives,
                **model_config
                ).to(self.args.device) 
        # print(self.args.device)
        if self.args.use_float16:
            self.model.half()
        elif self.args.use_float:
            self.model.float()
        else:
            self.model.double()
    ...
    def fit(self, data, info, train = True, config = None):
        ...
        whichFloat = torch.float16 if self.args.use_float16 else (torch.float32 if self.args.use_float else torch.float64)
        train_dataset = TensorDataset(torch.tensor(X_train, dtype=whichFloat), torch.tensor(y_train, dtype=whichFloat))
        self.train_loader = DataLoader(train_dataset, batch_size=self.args.batch_size, shuffle=True, drop_last=True)

        if X_val is not None and y_val is not None:
            val_dataset = TensorDataset(torch.tensor(X_val, dtype=whichFloat), torch.tensor(y_val, dtype=whichFloat))
        ...
        self.scheduler = CosineAnnealingLR(self.optimizer, T_max=self.args.max_epoch * len(self.train_loader), eta_min=1e-8) 
        time_cost = 0
    ...
    def predict(self, data, info, model_name, importance = False):
        N, C, y = data
        device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        self.model.load_state_dict(torch.load(osp.join(self.args.save_path, model_name + '-{}.pth'.format(str(self.args.seed))), map_location=device)['params'])
        ...
        if(importance):
            y_pred = test_logit.cpu().numpy().squeeze()
            return y_pred
        
        test_label__ = test_label.int() if self.is_multiclass else test_label.unsqueeze(1)
    ...
    def train_epoch(self, epoch):
        ...
        for i, (X, y) in enumerate(self.train_loader, 1):
            ...
            y = y.int() if self.is_multiclass else y.unsqueeze(1)
            X, y = X.to(self.args.device), y.to(self.args.device)
            if(self.args.objectives == 2):
                y = y.squeeze(1)
